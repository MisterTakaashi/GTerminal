...
OS:NewCommand(":help", function(client, entity, arguments)
	...
	gTerminal:Broadcast(entity, "  Menu d'aide BackTrack");
	gTerminal:Broadcast(entity, "");
...
OS:NewCommand(":v", function(client, entity, arguments)
	local command = arguments[1];
	...
	if (!command) then
		gTerminal:Broadcast(entity, "Hexedit");
		gTerminal:Broadcast(entity, "  INFO:");
		gTerminal:Broadcast(entity, "    Assembleur de virus.");
		gTerminal:Broadcast(entity, "  AIDE:");
		gTerminal:Broadcast(entity, "    :v d <nom> - Supprimer un fichier/repertoire.");
		gTerminal:Broadcast(entity, "    :v n <nom_a_cacher> <virus> - Ecrire un virus.");
		gTerminal:Broadcast(entity, "    :v l - Lister tous les virus presents dans ce repertoire.");
		gTerminal:Broadcast(entity, "    :v rd <nom> - Lire le virus en sandbox.");
	elseif (command == "d") then
		local key = arguments[2];
	...
	elseif (command == "n") then
		...
		if (success) then
			gTerminal:Broadcast(entity, "Nouveau virus cree: '"..key.."'.", GT_COL_SUCC);
		end;
	elseif (command == "l") then
		gTerminal:Broadcast(entity, "../");
